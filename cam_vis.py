...
#itc_model.load_state_dict(torch.load('/root/autodl-tmp/UI_ITC/checkpoints/ckp_rico_epoch25.pt'))
print('model loaded')
...
final_img_1.save(args.vis_res_folder_path + f'{id}_{args.sentence}_vis_rico.png')
print('UI_ITC visualization done')

if args.do_compare==True:
    # get model and load weights 
    model, target_layer, reshape_transform = getCLIP(model_name=args.clip_model_name, gpu_id=args.gpu_id)
    itc_model = model
    itc_model.load_state_dict(torch.load('/root/autodl-tmp/UI_ITC/checkpoints/ckp_cla_sep_epoch30.pt'))
    print('model loaded')
    ...
    # compute cam
    final_img_3 = get_clipcam(cam, itc_model, get_mask_threshold(args.clip_model_name), 
                            image=img, sentence=args.sentence, 
                            DISTILL_NUM = args.distill_num, ATTACK = args.attack_type, GPU_ID = args.gpu_id)

    final_img_3.save(args.vis_res_folder_path + f'{args.sentence}_vis_UI_ITC_cla_sep.png')
    print('UI_ITC visualization done')

    print('repeat process for original clip vis...')
    ori_model, target_layer, reshape_transform = getCLIP(model_name=args.clip_model_name, gpu_id=args.gpu_id)
    # get CAM model based on itc model and cam method
    cam = getCAM(model_name=args.cam_model_name, 
                model=ori_model, target_layer=target_layer, 
                gpu_id=args.gpu_id, reshape_transform=reshape_transform)

    # compute cam
    final_img_2 = get_clipcam(cam, ori_model, get_mask_threshold(args.clip_model_name), 
                                image=img, sentence=args.sentence, 
                                DISTILL_NUM = args.distill_num, ATTACK = args.attack_type, GPU_ID = args.gpu_id)
    final_img_2.save(args.vis_res_folder_path + f'{args.sentence}_vis_orig_clip.png')
